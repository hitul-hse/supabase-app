 ...
 * statements ranked with calls x mean as their second line. Gate runs and
 * request timings are shown as their reasons -- they are n/a today and
 * hiding them would suggest they exist; they sit under the statements so the
 * two half-width cards carry the same content height.
 */
  ...
  return (
    <section data-section="efficiency" style={SECTION_STYLE} className="grid grid-cols-1 gap-[var(--card-gap)] lg:grid-cols-2">
      <Card as="div" className={`flex flex-col ${CARD_WITH_DRILL}`}>
        <CardHeader title={t("efficiency.title")} qualifier={t("efficiency.qualifier")} />
        {/* flex-1 + items-center: whatever height the statements card sets, the
            slack gathers around the gauges instead of as a blank band at the foot. */}
        <div className="grid flex-1 grid-cols-2 content-center gap-x-3 gap-y-6 px-4 pb-5 pt-1 sm:grid-cols-3 sm:gap-x-4">
          {/* Cache hit */}
            ...
            {cache.ok ? (
              <DrillTrigger drill={cache.drill} id="efficiency-cache" className={`${FIGURE_TRIGGER} flex justify-center py-1`}>
                <Gauge
                  ...
                  max={100}
                  tone={cache.tone}
                  centre={fmt1(cache.pct)}
                  unit="%"
                  centreLabel={t("efficiency.cacheHit")}
                  label={t("efficiency.cacheLabel", { pct: fmt1(cache.pct) })}
                  width={GAUGE}
                />
              </DrillTrigger>
            ) : (
            ...
            {connections.ok ? (
              <DrillTrigger drill={connections.drill} id="efficiency-connections" className={`${FIGURE_TRIGGER} flex justify-center py-1`}>
                <Gauge
                  ...
                  centreLabel={t("efficiency.connections")}
                  label={t("efficiency.connLabel", { active: connections.active, max: connections.max })}
                  width={GAUGE}
                />
              </DrillTrigger>
            ) : (
          ...
          <div className="col-span-2 flex flex-col gap-1.5 py-1 sm:col-span-1" data-metric="db-latency" data-stat-tile>
        ...
        <ChartNote>
          {statsReset ? t("efficiency.statsNote", { at: fmtTime(statsReset, locale) }) : t("efficiency.statsNoteNoReset")}
        </ChartNote>
      </Card>
      ...
      <Card as="div" className={`flex flex-col ${CARD_WITH_DRILL}`}>
        ...
        <div className="px-4 pb-3">
          {statements.ok ? (
            <HBar label={t("efficiency.statementsLabel")} rows={statements.top} valueFormat={fmtMs} thickness={10} labelWidth="wide" />
          ) : (
