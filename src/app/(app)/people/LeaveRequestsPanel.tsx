...
import type { LeaveRequestRow } from "@/lib/queries/types";
  ...
  return (
    <div className="flex flex-col gap-3 border border-[var(--border)] bg-[var(--surface)] p-4">
      <div className="flex items-baseline gap-2.5">
        <span className="text-[12.5px] font-semibold text-[var(--text-primary)]">Leave requests</span>
      ...
      <div className="flex flex-col gap-2">
        ...
        {requests.map((r) => (
          <div key={r.id} className="flex items-center justify-between gap-2 border-b border-[#3a414c] pb-2 text-[12px]">
      ...
      {canRequest && (
        <form action={formAction} className="flex flex-col gap-2 border-t border-[var(--border)] pt-3">
          ...
          {state.status === "success" && (
            <p className="text-[11.5px] text-[var(--accent)]">{state.message}</p>
          )}
        </form>
      )}
    </div>
