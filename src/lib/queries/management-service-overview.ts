...
export type ServiceOverviewRow = {
  ...
  /** Null until a confirmed replacement relation exists. */
 ...
 * canonical Customer-Master Legal Entity. Replacement stays null until a
 * confirmed replacement relation exists; `person_assignments` alone is not
 * treated as a replacement relation. Reteach is a separate USERS concern and
 * has no source field in the current model, so no fabricated user row is emitted.
 */
export async function getManagementServiceOverview(
  supabase: SupabaseTyped,
): Promise<ServiceOverviewRow[]> {
  try {
    const [{ data: projects }, { data: assignments }, timeProjects, customerMaster] = await Promise.all([
    ...
    return [...aggregates.entries()]
      ...
      .map(([service, aggregate]) => ({
        ...
        // No confirmed replacement relation exists in the current schema.
