...
export function FactorialHoursPanel({ report }: { report: FactorialHoursReport }) {
  ...
  const columns: Column<PersonComparison>[] = [
    ...
    },
    {
      ...
      descFirst: true,
      // Present and Logged are the pair this table compares, so they are the
      // two figures at full contrast; the derived ones step down a level.
      cell: (p) => <span className="font-mono text-[12px] text-[var(--text-primary)]">{h(p.presentHours)}</span>,
    ...
    },
    {
      ...
      descFirst: true,
      cell: (p) => <span className="font-mono text-[12px] text-[var(--text-primary)]">{h(p.loggedHours)}</span>,
      csv: (p) => p.loggedHours ?? "",
    ...
    },
    {
      ...
      descFirst: true,
      cell: (p) => <span className="font-mono text-[12px] text-[var(--text-secondary)]">{h(p.billableHours)}</span>,
