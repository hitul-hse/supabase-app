...
const REVIEW_CASE_TYPES: ReviewCaseType[] = ["LEXWARE_REFERENCE_CONFLICT", "ALIAS_REVIEW", "LOCATION_REVIEW", "HISTORICAL_SOURCE_REVIEW", "CUSTOMER_MASTER_REVIEW"];
...
function CaseRow({ reviewCase, active, href }: { reviewCase: ReviewCase; active: boolean; href: string }) {
  return <Link href={href} scroll={false} className={`block px-4 py-3 transition-colors hover:bg-[var(--surface-hover)] ${active ? "bg-[var(--accent-wash)]" : ""} ${reviewCase.priority === "P0" ? "border-l-2 border-[var(--critical)]" : ""}`}><div className="grid gap-3 lg:grid-cols-[minmax(180px,1.1fr)_minmax(90px,0.5fr)_minmax(120px,0.8fr)_70px_minmax(120px,0.8fr)_minmax(180px,1.2fr)] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-sm font-semibold text-[var(--text-primary)]">{reviewCase.title}</h3><StatusBadge status={reviewCase.case_type} tone="info" /></div><p className="mt-1 font-mono text-[10px] text-[var(--text-faint)]">CASE {reviewCase.case_key}</p></div><div><span className="mr-2 font-mono text-[9px] text-[var(--text-faint)] lg:hidden">PRIORITÄT</span><StatusBadge status={reviewCase.priority} tone={priorityTone(reviewCase.priority)} /></div><div><span className="mr-2 font-mono text-[9px] text-[var(--text-faint)] lg:hidden">STATUS</span><StatusBadge status={reviewCase.status} tone={reviewCase.status === "RESOLVED" ? "positive" : reviewCase.status === "OPEN" ? "critical" : "warning"} /></div><div className="font-mono text-xs text-[var(--text-primary)]"><span className="mr-2 text-[9px] text-[var(--text-faint)] lg:hidden">RECORDS</span>{reviewCase.records.length}</div><div className="min-w-0 truncate font-mono text-[10px] text-[var(--text-muted)]"><span className="mr-2 text-[9px] text-[var(--text-faint)] lg:hidden">QUELLE</span>{reviewCase.sheet_names.join(" · ")}</div><div className="min-w-0 text-xs text-[var(--text-secondary)]"><span className="mr-2 font-mono text-[9px] text-[var(--text-faint)] lg:hidden">REASON</span>{reviewCase.review_reason}</div></div><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[10px] text-[var(--text-muted)]"><span>{reviewCase.review_statuses.join(" · ")}</span><span>{reviewCase.resolution_statuses.join(" · ")}</span></div></Link>;
}
