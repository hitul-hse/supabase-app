...
/*
 * A labelled block, so an early exit can stop the run WITHOUT process.exit().
 *
 * process.exit() tears the event loop down on top of the supabase-js client's
 * still-open undici keep-alive sockets, which on Windows trips
 *   Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), file src/win/async.c
 * -- the same libuv teardown assert 0d02bd5 fixed in
 * check-management-contract-hours-live. Observed here on 2026-08-28: the script
 * printed "Nothing to purge." and then died 0xC0000409. A cleanup tool that
 * reports success and then crashes reads as a failed cleanup.
 *
 * `break purge` leaves the block, lets Node drain those sockets, and exits on its
 * own with whatever process.exitCode holds.
 */
purge: {
  ...
  for (let page = 1; ; page += 1) {
    ...
    if (error) { console.log(`FAIL: could not list users -- ${error.message}`); process.exitCode = 1; break purge; }
  ...
  if (candidates.length === 0) {
    ...
    process.exitCode = 0; break purge;
  }
  ...
  for (let i = 0; i < ids.length; i += 200) {
    ...
    if (error) {
      ...
      process.exitCode = 1; break purge;
    }
    profiles.push(...(data ?? []));
  }
  ...
  /*
   * exitCode, NOT process.exit() -- the same libuv teardown assert 0d02bd5 fixed in
   * check-management-contract-hours-live. Observed here on 2026-08-28: this script
   * printed "Nothing to purge." and then died with
   *   Assertion failed: !(handle->flags & UV_HANDLE_CLOSING), file src\winsync.c
   * because process.exit() tears the loop down on top of the supabase-js client's
   * still-open undici keep-alive sockets. A cleanup tool that reports success and
   * then exits 0xC0000409 reads as a failed cleanup.
   */
  process.exitCode = leftover === 0 && failures.length === 0 ? 0 : 1;
